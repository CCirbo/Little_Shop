    ...
    describe '#class methods' do 
      ...
      describe '#sort_and_filter' do
        it 'filters by name only' do
          params = { name: "Mouse" }
          expected = [@item_1]
          result = Item.sort_and_filter(params).to_a # Convert to array
        ...
        # it 'filters by both min_price and max_price' do
        #   params = { min_price: 20, max_price: 120 }
        #   expected = [@item_1, @item_2, @item_3].map { |item| [item.name, item.unit_price] }
        #   result = Item.sort_and_filter(params).map { |item| [item.name, item.unit_price] }
