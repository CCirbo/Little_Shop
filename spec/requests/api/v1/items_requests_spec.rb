...
RSpec.describe "Items endpoints", type: :request do
  ...
  describe "Sad-Path Fetch One Item" do
  ...
  describe "Sad-Path Update New Item " do
    it "returns a 404 error if the item does not exist" do
      Merchant.create!(id: 1, name: "Test Merchant")
      item_params = {name: "Tray"}
      headers = {"CONTENT_TYPE" => "application/json"}
      
      patch "/api/v1/items/9999", headers: headers, params: JSON.generate({item: item_params}) 
  ...
  describe "Using The Find_all Controller Action (un-RESTful route)" do
    ...
    it "Sad-Path CANNOT return items when a query includes both a name and a min_price query simultaneously" do
    ...
    it "Sad-Path CANNOT return items when a query includes both a name and a max_price query simultaneously" do
    ...
    it "Sad-Path does NOT return a 404 status code if no items are returned" do
